  ...
  <div>
    ...
    <div class="main">
      <table>
        ...
        <tr v-for="(graduateIndividual, index) in graduateIndividuals" :key="index">
          <td>{{graduateIndividual.targetNum}}</td>
          <td>{{graduateIndividual.dachengdu}}</td>
          <td>{{graduateIndividual.ispass==1 ? "是":"否"}}</td>
        </tr>
      </table>
    </div>
  </div>
...
<script>
import { reqCourses, reqGraduateIndividual, reqTargets } from "../api";
...
export default {
  ...
  methods: {
    ...
    async getTotalAchievements() {
      let result = await reqGraduateIndividual(this.courseName, this.version);
      if (result.status === 200) {
        this.graduateIndividuals = result.data;
      }else{
        this.graduateIndividuals = '';
      }
    }
  },
